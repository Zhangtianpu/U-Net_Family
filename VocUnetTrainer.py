...
from Dataset.VOCDatasetLoader import  VOCSegmentationDataset
import argparse
import numpy as np
...
from BasicTrainer.BasicTrainer import Trainer
import Utils
...
def worker(rank,word_size,config_file):
    ...
    # load dataset
    trainDataset = VOCSegmentationDataset(is_train=True, crop_size=(320, 480), voc_root=data_config.get('data_folder'))
    testDataset = VOCSegmentationDataset(is_train=False, crop_size=(320, 480), voc_root=data_config.get('data_folder'))
    ...
    unet = UNet(channel_list=model_config.get("channel_list"),
                ...
                residual_connection=train_config.get('residual_connection'),
                forward_bn=train_config.get('forward_bn')).cuda()

    #to alter BatchNorm in UNet with SyncBatchNorm
    unet=nn.SyncBatchNorm.convert_sync_batchnorm(unet)
    torch.nn.parallel.DistributedDataParallel(module=unet,
    ...
    optimizer = optim.Adam(params=unet.parameters(), lr=train_config.get('base_lr'), eps = train_config.get('epsilon'))
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=train_config['steps'], gamma=train_config['lr_decay_ratio'])
    ...
    trainer.train()
    Utils.save_logs(data=trainer.logs,folder_path=train_config.get('experiment_result_folder'),filename=train_config.get('experiment_result_file'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file',type=str,default='./config/voc_config_unet.yml')
