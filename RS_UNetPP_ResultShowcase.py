...
def case_study_display(data_config, model_config, train_config,
    ...
    model = UNetPP(channel_list=model_config.get("channel_list"),
                ...
                forward_bn=model_config.get('forward_bn'),
...
if __name__ == '__main__':
    config_file = './config/RS_UnetPP_config.yml'
    showcase_stored_folder='./ExperimentImages/RS_UNetPP'
    ...
    plot_curve(X,train_loss,test_loss,"Learning Curve","Epoch","Loss",is_save=True,path=os.path.join(showcase_stored_folder,'LearningCurve.png'))
    plot_curve(X,train_acc,test_acc,"Accuracy Curve","Epoch","Accuracy",is_save=True,path=os.path.join(showcase_stored_folder,'AccuracyCurve.png'))
    plot_curve(X, train_miou, test_miou, "Mean Intersection Over Union", "Epoch", "MIOU",is_save=True,path=os.path.join(showcase_stored_folder,'MIOU.png'))

    case_study_display(data_config=data_config, train_config=train_config,
                       model_config=model_config, sample_number=6, epoch=195,
