...
from BasicTrainer.Unet3PBasicTrainer import Trainer
import Utils
...
def worker(rank, word_size, config_file):
    ...
    create U-Net model
    """
    unet3P = UNet3P(channel_list=model_config.get("channel_list"),
                    out_channel=model_config.get("output_channel"),
                    deep_supervision=model_config.get('deep_supervision'),
                    CGM=model_config.get('CGM'),
                    bias=train_config.get("bias")).cuda()

    # to alter BatchNorm in UNet with SyncBatchNorm
    unet3P = nn.SyncBatchNorm.convert_sync_batchnorm(unet3P)
    torch.nn.parallel.DistributedDataParallel(module=unet3P,
    ...
    optimizer = optim.Adam(params=unet3P.parameters(), lr=train_config.get('base_lr'), eps=train_config.get('epsilon'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file', type=str, default='./config/RS_Unet3P_config.yml')
