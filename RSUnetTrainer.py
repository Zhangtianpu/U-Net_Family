...
from MyLoss.BCEWithDice import BCEWithDiceLoss_
import argparse
import numpy as np
...
from BasicTrainer.UnetBasicTrainer import Trainer
import Utils
...
def worker(rank,word_size,config_file):
    ...
    # load dataset
    trainDataset = RemoteSensingData(data_folder=data_config.get('data_folder'),resize=data_config.get('resize'),train=True)
    testDataset = RemoteSensingData(data_folder=data_config.get('data_folder'),resize=data_config.get('resize'),train=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file',type=str,default='./config/RS_Unet_config.yml')
